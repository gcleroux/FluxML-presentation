...
epochs::Int64 = 30 			# Nb. epochs

# ╔═╡ 32abe461-f327-4dad-9b16-67dc55e2ae61
...
let	
	...
	samples_x, _ = MNIST.traindata(Float32, 1:9)
	
	# Converting samples to grayscale images
	imgs = [Gray.(i) for i in samples_x]
...
@bind drawing HTML("""
<div id=parent>
	<canvas id=canvas width=80px height=80px></canvas>
	<button id=clearButton>clear</button>
</div>
	
<script>
	const canvasWidth = 80, canvasHeight = 80, background = "#f1f1f1";
	...
	c.addEventListener('mousemove', (e) => {
		if(drawing) {
			ctx.beginPath();
			ctx.arc(e.offsetX, e.offsetY, 2, 2, 2 * Math.PI);
...
function convert_drawing(d)
	...
	# Reduce the matrix's size to 28x28
	mat = imresize(mat, (28, 28))
	mat = reshape(mat, (28, 28, 1, 1))
	
	return mat
end

# ╔═╡ ccb676ce-8c53-452b-a076-92e23f2e7049
### This is only here to visualize the drawing in a matrix of size 28x28
### and should be removed for the final revision

img = imrotate(Gray.(convert_drawing(drawing)[:, :, 1, 1]), -π/2)
